...
`muxm` ships eight opinionated profiles, listed below from highest fidelity to widest
...
E-AC-3 audio (with Atmos JOC when present), AAC stereo fallback, and
...
### 4) `atv-directplay-animation` — Anime for Apple TV Direct Play
...
### 5) `streaming` — Modern HEVC Streaming
...
### 6) `animation` — Anime & Cartoon Optimized
...
### 7) `universal` — Universal Compatibility
...
### 8) `youtube-upload` — YouTube Upload Prep
