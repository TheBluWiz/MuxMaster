...
- **New profile `youtube-upload`** — H.264 high-profile master-quality encode for YouTube ingestion. CRF 16, preset `slow`, x264 params `profile=high:rc-lookahead=60:aq-mode=2:aq-strength=1.0`. Forces AAC stereo at 256 k, burns forced subtitles, exports full subtitles as external SRT sidecars, strips non-essential metadata, keeps chapters. No tone-mapping (YouTube processes HDR natively); HDR10 metadata is preserved as-is. Container: MP4. DV layer disabled. Registered in `--help`, embedded and installed man pages, tab completions, `--create-config`, `_profile_comment`, and conflict warnings (warns on `--audio-lossless-passthrough` and `--output-ext mkv`). Profile comment: "Your master, YouTube's problem."
...
- **`--create-config` override values not applied** — The override block in `_create_config_emit` was adding variable names to the `_active` set and setting override values, but the profile's `case` block was unconditionally adding ALL profile variables to `_active`, causing them to appear uncommented even without overrides. Fixed by removing the unconditional `_active` population from the profile block — `_active` is now only populated by actual CLI overrides.
